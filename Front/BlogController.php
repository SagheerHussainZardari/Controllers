...
class BlogController extends Controller
{
    public function index()
    {
        ...
        $blog_items = DB::table('blogs')->orderby('id', 'desc')->paginate(5);
        $blog_items_no_pagi = DB::table('blogs')->orderby('id', 'desc')->get();
        $categories = DB::table('categories')->get();

        $page_title = __('customer.blogs');

        return view('pages.blogs', compact('blog', 'g_setting', 'blog_items', 'categories', 'blog_items_no_pagi', 'page_title'));
    }
    ...
    public function detail($slug)
    {
        ...
        $page_title = __('customer.blog_details');


        return view('pages.blog_detail', compact('g_setting', 'blog_detail', 'blog_items', 'blog_items_no_pagi', 'categories', 'comments', 'page_title'));
    }
